...
namespace WordWorld
{
    ...
    {
        internal static Dictionary<Type, (Func<IDrawable, string[]>, Action<IDrawable, FLabel[]>, Action<IDrawable, FLabel[], RoomCamera.SpriteLeaser, float, Vector2>)> RegisteredClasses = [];
        ...
        /// <param name="drawLabelsFunc">Action that will be called after DrawSprites. Parameters: the IDrawable, the FLabels, the sprite leaser, timeStacker, and camPos.</param>
        /// <exception cref="ArgumentException">Throws if the type passed into the function is not an IDrawable.</exception>
        public static void RegisterItem(Type type, Func<IDrawable, string[]> createLabelsFunc, Action<IDrawable, FLabel[]> initLabelsFunc, Action<IDrawable, FLabel[], RoomCamera.SpriteLeaser, float, Vector2> drawLabelsFunc)
        {
